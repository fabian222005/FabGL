...
{
    // ensure coordinates are within bounds and errase the garbage lines generated by moving or resizing windows 
    x2 +=1;
    uint8_t* draw_buffer = getDrawBuffer ();
...
void VGA::set_pixel(u_int x, u_int y, uint8_t r, uint8_t g, uint8_t b)
{
    ...
    if (y>_screenHeight)
        return;
    uint8_t* draw_buffer = getDrawBuffer ();
    draw_buffer[y*_screenWidth+x]=rgb_to_bits(r,g,b);
}
